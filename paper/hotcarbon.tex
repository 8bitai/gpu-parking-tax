...
NVIDIA shipped the \texttt{CUDA\_DISABLE\_PERF\_BOOST} environment variable in driver 580.105.08 (November 2025), intended to prevent idle-context clock boosting. We evaluate its effectiveness on H100 SXM (Hopper, driver 580.126.09) and A100 SXM (Ampere) under three conditions: (a)~bare idle (no CUDA context), (b)~baseline (CUDA context + \texttt{torch.empty} 16\,GB, default behavior), and (c)~same with \texttt{CUDA\_DISABLE\_PERF\_BOOST=1}. Each condition: \texttt{nvidia-smi} every 30\,s ($n=10$).

\begin{table}[t]
\caption{Effect of \texttt{CUDA\_DISABLE\_PERF\_BOOST} on H100 idle power ($n=10$ per condition, driver 580.126.09). The flag eliminates 100\% of the DVFS overhead.}
\label{tab:perf-boost}
\small
\begin{tabular}{lrrrr}
\toprule
\textbf{Condition} & \textbf{Power (W)} & \textbf{SM (MHz)} & \textbf{PState} & $\Delta$\textbf{P} \\
\midrule
Bare idle (no CUDA) & 73.7 $\pm$ 0.2 & 345 & P0 & --- \\
Baseline (ctx + 16\,GB) & 124.7 $\pm$ 1.9 & 1980 & P0 & +51.0 \\
Flag on (ctx + 16\,GB) & 73.4 $\pm$ 0.1 & 345 & P0 & $-$51.3 \\
\bottomrule
\end{tabular}
\end{table}

The flag \textbf{completely eliminates the parking tax} on H100. With \texttt{CUDA\_DISABLE\_PERF\_BOOST=1}, the SM clock remains at bare-idle 345\,MHz even with an active CUDA context and 16\,GB allocated. Power drops from 124.7\,W to 73.4\,W---indistinguishable from bare idle (73.7\,W). This confirms that the entire parking tax is a software policy artifact: the DVFS boost, not the CUDA context itself, is the sole source of idle overhead. The flag's effect persists through the full recording period with $\sigma = 0.06$\,W. The effect on inference latency under load remains to be characterized.
...
\paragraph{Design directions.} (1)~\textbf{Idle power states}: \texttt{CUDA\_DISABLE\_PERF\_BOOST} already eliminates 100\% of the H100 parking tax (\S\ref{sec:perf-boost}), proving the overhead is a software policy artifact. Remaining gaps: the flag requires per-process opt-in, and its latency impact under load is unknown. (2)~\textbf{Context pooling}: Share CUDA contexts via MPS~\cite{nvidia-mps} to amortize overhead. (3)~\textbf{Size-aware scheduling}: Preferentially cold-start small models (fast reload, same parking cost). (4)~\textbf{Breakeven-aware eviction}: A simple $T^*$-based policy saves 8--23\% energy in our simulations (\S\ref{sec:scheduler-note}).
