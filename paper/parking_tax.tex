...
NVIDIA shipped the \texttt{CUDA\_DISABLE\_PERF\_BOOST} environment variable in driver 580.105.08 (November 2025), intended to prevent the idle-context clock boosting behavior we measure. This is the most direct vendor-provided mechanism targeting the DVFS anomaly documented in \S\ref{sec:measurement} and by Patel et al.~\cite{patel-cal23}. We evaluate its effectiveness on H100 and A100 GPUs under three conditions:
...
Each condition follows the Phase~2 protocol: stabilize 60\,s, record \texttt{nvidia-smi} every 30\,s for 20~min ($n=40$). For the vLLM conditions, we additionally measure request latency (p50, p95, p99) over 10 requests to detect performance regressions introduced by the flag.

%% TODO: Replace with actual experimental results once collected.
%% Key questions this section must answer:
%%   1. Does the flag lower idle-with-context power on H100/A100?
%%   2. If yes, by how much? What SM clock does it settle at?
%%   3. Does it affect active-request latency (p50, p95, p99) under vLLM?
%%   4. Does the effect persist through the full 20-min idle period,
%%      or do driver heuristics override it?
%%   5. Does the flag interact with the performance state (PState)?

\begin{table}[t]
\caption{Effect of \texttt{CUDA\_DISABLE\_PERF\_BOOST} on idle-with-context power. Each cell: mean $\pm$ std over $n=40$ samples (20\,min). Placeholder---awaiting experimental data.}
...
%% TODO: Add vLLM latency comparison table:
%% | GPU | Condition | p50 (s) | p95 (s) | p99 (s) | Idle Power (W) |
