...
\begin{table}[t]
\caption{Effect of \texttt{CUDA\_DISABLE\_PERF\_BOOST} on idle-with-context power. Each cell: mean $\pm$ std. H100 SXM: quick-mode ($n=10$, 5\,min, driver 580.126.09); A100 SXM: pending.}
...
\midrule
\multirow{3}{*}{\shortstack[l]{H100 SXM\\(Hopper)}} & Bare idle & 73.7 $\pm$ 0.2 & 345 & P0 & --- \\
 & Baseline & 124.7 $\pm$ 1.9 & 1980 & P0 & +51.0 \\
 & Flag on & 73.4 $\pm$ 0.1 & 345 & P0 & $-$51.3 \\
\midrule
\multirow{3}{*}{\shortstack[l]{A100 SXM\\(Ampere)}} & Bare idle & --- & --- & --- & --- \\
 & Baseline & --- & --- & --- & --- \\
 & Flag on & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
...
\paragraph{H100 results.} On the H100 (driver 580.126.09), \texttt{CUDA\_DISABLE\_PERF\_BOOST=1} \textbf{completely eliminates the parking tax}. With the flag set, the SM clock remains at the bare-idle frequency of 345\,MHz---identical to the no-CUDA-context state---even though a CUDA context is active and 16\,GB of VRAM is allocated. Idle power drops from 124.7\,W (baseline) to 73.4\,W (flag on), a reduction of 51.3\,W that accounts for 100\% of the DVFS overhead measured in Phase~2 ($\Delta P_{\text{DVFS}} = 49.9$\,W in Table~\ref{tab:cross}). The flag-on power (73.4 $\pm$ 0.1\,W) is indistinguishable from bare idle (73.7 $\pm$ 0.2\,W), confirming that the entire parking tax is attributable to the DVFS boost and that the flag successfully suppresses it. The effect persists through the full recording period with no sign of driver heuristics overriding the setting. The within-phase standard deviation under the flag ($\sigma = 0.06$\,W) is lower than bare idle ($\sigma = 0.16$\,W), suggesting the flag produces a more thermally stable idle state.

%% TODO: A100 results pending. Run: python experiments/perf_boost.py --gpu 0
%% TODO: vLLM latency comparison pending (vLLM did not produce data in this run).
%% Key open question: does the flag affect inference latency when requests arrive?
