...
\begin{abstract}
The AI inference industry keeps models resident in GPU memory around the clock to avoid cold-start latency, treating idle power as a fixed cost of readiness. The structure of this cost has not been decomposed empirically, and not across GPU architectures. We combine 18~days of production telemetry (\num{335267} idle samples, 14~H100 GPUs) with controlled dose-response experiments on four architectures---H100 (HBM3), A100 (HBM2e), L40S (GDDR6), and B200 (Blackwell, HBM3e)---to isolate the CUDA context as the variable that sets idle power. Idle power is \emph{piecewise constant}: creating a CUDA context forces a DVFS transition that adds $26$--$66$\,W over bare idle, while the marginal VRAM effect is statistically indistinguishable from zero ($|\beta| < 0.02$\,W/GB) on every device tested. On the H100, a real Qwen2.5-7B model idles within 0.03\,W of an equal-context empty allocation. NVIDIA's \perfboost driver flag returns idle-with-context power toward the bare-idle floor, recovering roughly 95--100\% of the overhead, with no steady-state latency penalty measured under vLLM on the H100 and A100. On a 4$\times$H100~SXM node the tax multiplies linearly at ${\sim}52$\,W per GPU across tensor, data, and mixed parallelism---so a 4-GPU node wastes ${\sim}215$\,W idle---while the first-request cold-start penalty stays constant at ${\sim}150$\,ms regardless of GPU count. Setting the flag at process start is a near-zero-cost mitigation: the DVFS governor drops clocks to bare idle within 2\,s of the last request and ramps on demand, the only cost being a one-time ${\sim}150$\,ms first-request penalty that does not compound with~$N$. At data-center scale, a mid-range scenario places the recoverable overhead near \num{460}\,GWh per year (${\sim}180$\,kT CO$_2$).
\end{abstract}
...
This paper addresses that gap. We ask whether storing model weights in GPU VRAM incurs a measurable marginal idle energy cost, and whether the answer depends on GPU architecture. We answer with four complementary studies:
...
\begin{enumerate}
    ...
    \item \textbf{Controlled experiments} (Phase~2): Within-subject dose-response studies on four GPU architectures---H100 (HBM3), A100 (HBM2e), L40S (GDDR6), and B200 (Blackwell, HBM3e)---systematically varying VRAM from 0 to 40--72\,GB, validated with real model weights.
    \item \textbf{Vendor mitigation} (Phase~3): Evaluation of NVIDIA's \perfboost driver flag on H100 (Hopper) and A100 (Ampere), measuring idle power elimination, steady-state latency, and first-request DVFS ramp under both synthetic and vLLM workloads.
    \item \textbf{Multi-GPU scaling} (Phase~4): 9~conditions on a 4$\times$H100~SXM node testing TP=2, TP=4, DP=2, and TP=2$\times$DP=2 with and without \perfboost, measuring idle power, warm latency, DVFS decay, and cold-start ramp per parallelism strategy.
\end{enumerate}
...
B200 180GB SXM & HBM3e & 1000\,W & 0--32\,GB & 18 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
    ...
    \label{eq:bound}
\end{equation}
This converts our measurement limitation into a \emph{result constraint}. Regarding Burtscher et al.'s~\cite{burtscher-power} finding that \texttt{nvidia-smi} undersamples: our within-phase standard deviations confirm idle power is stable enough that subsampling does not bias means. Complementary per-instruction energy models such as Wattchmen~\cite{wattchmen} achieve finer-grained decomposition; extending our idle-state analysis with such methods is a promising direction. For the shorter Blackwell and idle-recovery runs, and in the dose-response figures, we report the per-phase \emph{median} rather than the mean of Equation~\ref{eq:sampling}: the median is insensitive to the occasional transient DVFS boost a single sample can capture (one such sample appears in the Blackwell flag run), and on the 20-minute primary phases ($n=40$) the mean and median agree to within 1\,W.

\subsection{Phase~3: Vendor Mitigation Setup}
\label{sec:perf-boost-setup}

NVIDIA added the \perfboost environment variable in the 580 driver series (CUDA Toolkit 13.2, late 2025; publicly noted at driver 580.105.08), intended to prevent the idle-context clock boosting behavior documented by Patel et al.~\cite{patel-cal23} and confirmed in our Phase~1 and Phase~2 results. We evaluate its effectiveness on H100 80GB SXM (Hopper, driver 580.126.09) and A100 80GB SXM4 (Ampere, driver 580.126.16) under three conditions:

\begin{enumerate}
    \item \textbf{Baseline}: CUDA context + \texttt{torch.empty} 16\,GB, default driver behavior.
    \item \textbf{Flag on}: Same as (1) with \perfboost\texttt{=1} in the process environment.
...
\subsection{Phase~4: Multi-GPU Experimental Setup}
...
\begin{table}[t]
\caption{Cross-architecture parking tax. Power values are per-phase means (medians for B200; \S\ref{sec:measurement}). $\beta$ is the linear regression slope across CUDA-active phases. All GPUs at 0\% utilization.}
\label{tab:cross}
\scriptsize
...
CUDA ctx power (W) & 121.7 & 80.0 & 102.1 & 233.6 \\
SM clk, ctx (MHz) & 1980 & 1410 & 2520 & 1965 \\
\midrule
\textbf{Context overhead (W)} & \textbf{+49.9} & \textbf{+26.3} & \textbf{+66.4} & \textbf{+51.9} \\
Context (\% of TDP) & 7.1\% & 8.8\% & 19.0\% & 5.2\% \\
\midrule
Max VRAM tested (GB) & 64 & 72 & 40 & 32 \\
Power range, VRAM (W) & +0.34 & $-$0.09 & +0.08 & +0.60 \\
$\beta$ (W/GB) & $-$0.002 & $-$0.001 & $-$0.002 & $+$0.019 \\
$p$-value ($\beta = 0$) & 0.404 & 0.002$^\dagger$ & 0.794 & $<$0.001$^\dagger$ \\
$p_{\text{TOST}}$ ($|\beta| < 0.1$) & $10^{-9}$ & $<0.001$ & $<0.05$ & $<0.001$ \\
\midrule
\textbf{Context \% of tax} & \textbf{$>$99\%} & \textbf{$>$99\%} & \textbf{$>$99\%} & \textbf{$>$99\%} \\
\bottomrule
\end{tabular}
\par\vspace{2pt}
\parbox{\columnwidth}{\footnotesize $^\dagger$Slope significant but a temporal/thermal artifact of the sequential sweep (A100 negative, B200 positive), $<$1\,W total and far below the 0.1\,W/GB bound; see \S\ref{sec:a100-note}. B200 values are per-phase medians (transient-robust); others are means, agreeing within 1\,W.}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{Parking tax decomposition. Gray: base idle power. Color: CUDA context overhead (98--$>$99\% of the tax on every architecture). The VRAM term is too small to render.}
    \label{fig:decomp}
\end{figure}
...
\paragraph{Blackwell (HBM3e).} On the B200 (Blackwell, HBM3e) we ran the same dose-response over four VRAM levels (3-minute phases, $n=18$; Figure~\ref{fig:b200}). Creating a context raises idle power from 182\,W (120\,MHz) to 234\,W (1965\,MHz)---a 52\,W tax at 0\% utilization---and power stays flat within 1\,W across 0--32\,GB, matching the other three architectures. The boosting default is present on this current-generation part.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{b200_dose_response.png}
    \caption{Dose-response on an NVIDIA B200 (Blackwell, HBM3e); four VRAM levels, $n=18$ per phase. Creating a CUDA context adds a 52\,W tax (182$\to$234\,W) at 0\% utilization; power then stays flat within 1\,W across 0--32\,GB, matching the other three architectures.}
    \label{fig:b200}
\end{figure}

\subsection{Real Model Validation}

The Phase~2 experiments use \texttt{torch.empty} tensors to isolate the VRAM effect from model-specific CUDA state. To confirm that real model weights---with their CUDA kernels, cuDNN handles, and weight tensors---do not change the result, we loaded a production Qwen2.5-7B model (fp16~\cite{qwen2}) and measured idle power (Table~\ref{tab:validation}).
...
On the H100, Qwen2.5-7B idles at 121.48\,W---within 0.03\,W of the \texttt{torch.empty} baseline ($n=30$, $\sigma = 0.11$\,W), and far below the 26--66\,W CUDA context overhead. A real 7B model loaded on the L40S idles at 105.85\,W, in line with the context-active idle level measured for that architecture in Phase~2; absolute idle power varies across GPU instances, tracking the $\sim$23\,W inter-node spread observed in Phase~1, while the within-instance VRAM invariance holds regardless. Whether a GPU holds a real 7B model or an equivalent block of empty memory, the idle cost is set by whether a context is resident, not by the model.
...
Given a parking tax of $P_{\text{park}}$ watts, an operator may instead evict the model and cold-start it on the next request. The energy breakeven between keeping warm and cold-starting is:
\begin{equation}
...
\paragraph{A third option.} The breakeven analysis assumes a binary keep-warm-or-evict decision. A third option keeps the model loaded while removing the parking tax at the driver level, paying only a transient ramp cost per idle-to-active transition instead of a full cold start. We evaluate it next.

\subsection{Phase~3: Vendor Mitigation}
\label{sec:perf-boost}

NVIDIA's \perfboost environment variable (added in the 580 driver series; tested at $\geq$580.105.08, November 2025) is the most direct vendor-provided mechanism targeting the DVFS anomaly. Using the experimental protocol described in \S\ref{sec:perf-boost-setup}, we evaluate its effectiveness on H100 (Hopper) and A100 (Ampere).

\begin{table}[t]
\caption{Effect of \perfboost on idle-with-context power. H100/A100: mean $\pm$ std ($n=40$, 20-min phases; drivers 580.126.09 and 580.126.16). L40S and B200: shorter idle-only protocol (per-phase medians).}
...
\midrule
\multirow{3}{*}{\shortstack[l]{L40S\\(Ada Lovelace)}} & Bare idle & 35.6 $\pm$ 0.1 & 210 & --- & --- \\
 & Baseline & 102.7 $\pm$ 0.7 & 2520 & --- & +67.1 \\
 & Flag on & 36.7 $\pm$ 0.1 & 210 & --- & $-$66.0 \\
\midrule
\multirow{3}{*}{\shortstack[l]{B200\\(Blackwell)}} & Bare idle & 181.7 $\pm$ 0.2 & 120 & --- & --- \\
 & Baseline & 233.5 $\pm$ 0.2 & 1965 & --- & +51.8 \\
 & Flag on & 181.8 $\pm$ 0.3 & 120 & --- & $-$51.7 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{The mitigation holds across four architectures.} Beyond the H100 and A100 (Table~\ref{tab:perf-boost}), we measured the flag's idle-power effect on the L40S and B200 with an idle-only protocol (Figure~\ref{fig:flag-recovery}). On the L40S---the highest-tax device---context-active idle falls from 102\,W (2520\,MHz) to 36.8\,W (210\,MHz), the bare-idle floor: essentially the full tax recovered. On the Blackwell B200 the flag returns idle power to the same floor: with the flag set, median context-active idle falls from 234\,W to 182\,W (the bare-idle floor), and the SM clock dropped to 120\,MHz in 71 of 72 context-active samples, with one transient boost during the 4\,GB phase. Across all four architectures the flag recovers roughly 95--100\% of the context overhead.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{flag_recovery_across_architectures.png}
    \caption{The \perfboost flag returns context-active idle power toward the bare-idle floor on all four architectures (H100, A100, L40S, B200). Recovery is roughly 95--100\% on all four; on the B200 the flag returns context-active idle to the 120\,MHz bare floor (234$\to$182\,W).}
    \label{fig:flag-recovery}
\end{figure}
...
\paragraph{Inference latency.} We test whether the flag adds latency when requests arrive and the GPU must ramp clocks from idle, using a focused retest protocol: start vLLM, send 3 warmup requests, idle for 60\,s (allowing clocks to drop under the flag), then send 5 ``cold'' requests followed by 50 ``warm'' requests back-to-back.
...
\paragraph{Throughput under load.} To check that the flag does not cap active clocks, we ran a high-concurrency decode benchmark on the H100 (Qwen2.5-7B, 512 requests at concurrency~64; Figure~\ref{fig:throughput}). In a single run, aggregate decode throughput was 9287\,tok/s at baseline and 9355\,tok/s with the flag (a 0.7\% difference); repeated trials would be needed to characterize run-to-run variance. In both conditions the GPU held the same 1980\,MHz load SM clock---its full boost frequency---so the flag does not cap active clocks. It suppresses only the idle boost.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{throughput_flag_comparison.png}
    \caption{H100 decode throughput and load SM clock, flag off vs.\ on (Qwen2.5-7B, 512 requests at concurrency~64, single run). Throughput is 9287\,tok/s (baseline) and 9355\,tok/s (flag); both hold the 1980\,MHz load clock. The flag does not cap active clocks.}
...
\subsection{Phase~4: Multi-GPU Scaling}
\label{sec:multi-gpu}
...
\emph{Note:} The \perfboost flag (\S\ref{sec:perf-boost}) is the preferred mitigation when available (NVIDIA GPUs, driver in the 580 series; tested at $\geq$580.105.08). The breakeven scheduler below is a \textbf{fallback} for deployments without flag access---older drivers, non-NVIDIA hardware, or environments where the flag cannot be set.
...
\paragraph{Cross-architecture consistency.} The parking tax is a step function of CUDA context on H100 (Hopper), A100 (Ampere), and L40S (Ada Lovelace), and on Blackwell (B200)---four microarchitectures with different memory technologies, process nodes, and TDP budgets. The magnitude varies (26--66\,W), but the structure is identical: $\partial P / \partial C \gg \partial P / \partial V$. With $n = 1$ device per architecture in Phase~2 (with an $n=18$ protocol on the B200), we cannot claim universal invariance, but the consistency---combined with the physics argument of Equation~\ref{eq:mem-prediction}---points to a design-space constraint rather than a device-specific artifact. Phase~1 bounds inter-device variation directly: regression across all 14~H100 GPUs yields slopes indistinguishable from zero on every device, even as intercepts vary by $\sim$23\,W from silicon binning and cooling. This pattern is consistent with $\beta \approx 0$ being device-invariant rather than an artifact of any particular GPU.
...
Our experiments cover TP and DP on a single NVLink node. Two extrapolations follow. Under pipeline parallelism (PP), stages communicate via point-to-point send/recv rather than all-reduce; the mechanism we identify is per-CUDA-context and independent of communication pattern (\S\ref{sec:multi-gpu}), so the per-GPU tax should be unchanged, though we have not measured it directly. For multi-node deployments, the DVFS governor on each GPU responds to local SM activity, not the interconnect fabric, so InfiniBand or RoCE should not change the per-GPU result. Under this hypothesis, a 64-GPU deployment across 8 nodes would waste $64 \times 52 \approx 3{,}328$\,W idle and recover the same via the flag. Both extrapolations are testable; we leave direct validation as future work.
