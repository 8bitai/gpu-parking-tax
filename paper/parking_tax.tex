...
The 2024 US Data Center Energy Usage Report estimates GPU idle power at $\sim$20\% of TDP~\cite{doe-report}. Patel et al.~\cite{patel-cal23} showed that CUDA contexts on A100 GPUs force higher clock frequencies even when idle. Neither study measured the marginal energy cost \emph{per GB of VRAM allocation}---the quantity needed for rational keep-warm decisions---nor did either test whether the result generalizes across GPU architectures and memory technologies.
...
\begin{equation}
    ...
    \label{eq:model}
\end{equation}
where $C \in \{0,1\}$ indicates a CUDA context, $V$ is VRAM allocated (GB), and $\beta$ is the marginal VRAM power coefficient. On all three devices, $|\beta| < 0.02$\,W/GB and we cannot reject $\beta = 0$ as a physically meaningful effect. The optimization priority is:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:mem-power}
\end{equation}
$P_{\text{background}}$ is constant whenever the memory clock runs. $P_{\text{refresh}}$ is proportional to \emph{capacity}, not occupancy---refresh circuitry sweeps all rows regardless of data content. $P_{\text{activate}}$ (row open/close) is zero at idle. This decomposition applies to both HBM (HBM3, HBM2e) and GDDR6, as all are DRAM-based with mandatory refresh. Therefore, at idle:
\begin{equation}
...
\paragraph{Infrastructure.} We instrumented 14 NVIDIA H100 80GB SXM5 GPUs across 2~nodes running Kubernetes. DCGM Exporter exposes metrics to Prometheus at 30-second intervals.
...
\begin{equation}
    ...
    \label{eq:bound}
\end{equation}
This converts our measurement limitation into a \emph{result constraint}. Regarding Burtscher et al.'s~\cite{burtscher-power} finding that \texttt{nvidia-smi} undersamples: our within-phase standard deviations confirm idle power is stable enough that subsampling does not bias means.
...
\begin{figure}[t]
    ...
    \caption{Parking tax decomposition. Gray: base idle power. Color: CUDA context overhead (98--$>$99\% of the tax). The VRAM component (red sliver, $<$1\,W) is invisible at this scale on all three architectures.}
    \label{fig:decomp}
\end{figure}
...
\paragraph{Model-size independence.} Because $P_{\text{park}}$ is a fixed cost (independent of $V$), $T^*$ depends only on loading power and time---not model size. Small models are the \emph{worst} candidates for always-on deployment: they reload in seconds yet pay the full parking tax.
...
Let $N$ denote total datacenter GPUs and $\rho$ the average utilization:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Fixed TTL}: Evict after a fixed idle timeout (5, 15, or 30 minutes).
...
\paragraph{Cross-architecture behavior.} The breakeven time varies by architecture: $T^* = 271$\,s (H100), 513\,s (A100), and 190\,s (L40S). The L40S, with the highest parking tax (66.4\,W), has the shortest breakeven time and the most to gain from eviction. With fast model loading (ServerlessLLM, $t_{\text{load}} = 8$\,s), $T^*$ drops to 50\,s (H100), enabling aggressive eviction even at moderate request rates.
...
\paragraph{Why $\Delta P_{\text{DVFS}}$ varies.} The L40S has the highest context overhead (66.4\,W, 19\% of TDP) despite having the lowest TDP (350\,W). This is because the L40S boosts to 2520\,MHz (vs.\ 1980\,MHz for H100, 1410\,MHz for A100). Higher idle clock frequency means higher idle power. The A100's modest 1410\,MHz idle clock explains its lower 26.3\,W overhead.
...
\paragraph{Reproducibility.} All data, analysis scripts, and experiment code are available at [anonymized for review].
