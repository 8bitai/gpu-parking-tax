...
\begin{table}[t]
\caption{vLLM inference latency with and without \texttt{CUDA\_DISABLE\_PERF\_BOOST}. Qwen2.5-7B (fp16), $n=50$ requests per condition, 128-token prompt / 128-token completion.}
\label{tab:latency}
\small
\begin{tabular}{llrrrr}
\toprule
...
\midrule
\multirow{2}{*}{H100 SXM} & Baseline & 533.3 & 533.0 & 536.5 & 537.4 \\
 & Flag on & 532.7 & 531.5 & 539.1 & 582.5$^\dagger$ \\
\midrule
\multirow{2}{*}{A100 SXM4} & Baseline & 777.3 & 777.4 & 778.3 & 778.6 \\
 & Flag on & 777.4 & 777.4 & 778.4 & 778.7 \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^\dagger$Single outlier (1/50 requests); 48/50 within normal range. Likely first-request clock ramp.}
\end{tabular}
\end{table}
...
\paragraph{Inference latency.} The critical question is whether the flag introduces a latency penalty when requests arrive and the GPU must ramp clocks from the idle state. Table~\ref{tab:latency} and Figure~\ref{fig:perf-boost-latency} show vLLM request latency ($n=50$, 128-token prompt, 128-token completion) with and without the flag. On the A100, latency is identical across all percentiles: mean 777.4\,ms (flag on) vs.\ 777.3\,ms (baseline), p50 unchanged, p99 within 0.1\,ms. On the H100, mean latency is unchanged ($-$0.6\,ms, $-$0.1\%) and p50 shows a negligible $-$1.5\,ms ($-$0.3\%) difference. The H100 p99 shows one outlier at 582.5\,ms (1/50 requests, $+$45\,ms over baseline p99), consistent with a one-time clock ramp on the first request; 48/50 requests fall within the normal range. This contrasts sharply with Lei et al.'s~\cite{lei-idle} frequency downscaling approach, which achieves 22--34\% idle power savings on L40S but with significant latency tradeoffs at active inference time. The \texttt{CUDA\_DISABLE\_PERF\_BOOST} flag eliminates the parking tax during idle periods while preserving full clock speeds during active inference---achieving the energy savings without the latency cost.
...
\begin{figure}[t]
    ...
    \caption{Inference latency with and without \texttt{CUDA\_DISABLE\_PERF\_BOOST}. Box plots with individual request latencies overlaid ($n=50$ per condition, Qwen2.5-7B via vLLM). On the A100, distributions are identical. On the H100, median and mean are unchanged; one outlier at 582\,ms (red dot) is consistent with a first-request clock ramp.}
