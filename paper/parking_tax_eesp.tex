...
\newcommand{\perfboost}{\texttt{CUDA\_\hspace{0pt}DISABLE\_\hspace{0pt}PERF\_\hspace{0pt}BOOST}\xspace}
...
\begin{abstract}
Data centers face growing pressure to reduce energy use, yet inference GPUs spend much of their time idle---holding a model in memory to stay responsive while serving no requests. The power a GPU draws in this state is commonly treated as a fixed cost of readiness. We show that most of it is not: it is an artifact of a driver default, and it is recoverable. Using 18~days of production telemetry from 14~H100 GPUs and controlled experiments on three GPU architectures spanning three memory technologies (HBM3, HBM2e, and GDDR6), we find that idle power does not depend on the model a GPU holds---a GPU holding a 79\,GB model and one holding 3\,MB draw the same---but on whether a CUDA context is resident, which forces full clock speed even at zero utilization and adds 26--66\,W per GPU. Because this power is spent with no work to perform, it is an anomaly rather than a genuine cost of serving. We confirm the cause with a controlled experiment: holding the model fixed while setting a single driver flag (\perfboost) returns idle power to its floor with no steady-state latency penalty. At data-center scale the overhead represents an estimated 92--\num{1745}\,GWh per year. It is the most prominent of a broader class of \emph{configuration-induced idle-power anomalies}: cases where a configuration setting, not the workload, holds idle power above its floor.
\end{abstract}
...
This paper asks whether that cost is unavoidable, and finds that most of it is not. We decompose idle power and show that it does not come from the resident model: across three memory technologies, allocated memory has no measurable effect, and a fully loaded GPU idles at the same power as an almost-empty one. What determines idle power is a single configuration state---whether a CUDA context is present---which pins the GPU at full clock frequency even when it performs no work, adding 26--66\,W. This overhead is therefore an anomaly rather than an intrinsic cost of serving; because it is a configuration artifact, we show it can be removed outright.
...
\begin{enumerate}
\item \textbf{Idle power is not a cost of the resident model.} We decompose idle power and find it invariant to allocated VRAM ($|\beta| < 0.02$\,W/GB, equivalent to zero under two one-sided tests) across HBM3, HBM2e, and GDDR6, in both production telemetry and controlled experiments, ruling out the resident model as its source (\S\ref{sec:model-doesnt-matter}).
\item \textbf{The overhead is a removable configuration artifact.} Idle power is a step function of CUDA-context presence. A controlled experiment that holds the model fixed and toggles a single driver flag removes the entire overhead (Hopper $-55$\,W, Ampere 96\%) with no steady-state latency penalty, establishing that the waste is an artifact rather than a physical cost (\S\ref{sec:control-plane}).
\item \textbf{The behavior is an intentional latency-oriented default.} It reduces first-request latency at the cost of continuous idle power; we quantify this trade-off and identify the regime---always-on serving---in which it is inappropriate (\S\ref{sec:default}).
...
The boost-on-context behavior was documented by Patel et al.~\cite{patel-cal23} in 2023; the anomaly is not new. What is new is the mitigation and the framing. NVIDIA added the \perfboost environment variable in the 580 driver series (CUDA Toolkit 13.2, late 2025; publicly noted at driver 580.105.08, November 2025). As \S\ref{sec:default} argues, the default is not an oversight but a deliberate latency-optimizing heuristic whose cost/benefit only recently tipped as always-on serving came to dominate GPU idle time; the flag is NVIDIA's tacit admission that one global default cannot serve both latency-critical interactive and always-on serving workloads.

\subsection{Other related work}
Zeus~\cite{zeus} optimizes energy for \emph{active} training. Patel et al.~\cite{patel-asplos24} characterize LLM inference power \emph{phases} during active execution. Burtscher et al.~\cite{burtscher-power} show \texttt{nvidia-smi} samples power for only ${\sim}25\%$ of runtime---a caveat we address with long, low-variance steady-state windows (\S\ref{sec:method}). Lei et al.~\cite{lei-idle} quantify how often execution-idle occurs; we isolate what the idle cost is a property of, bound the memory term, and prove the mechanism causally. Cold-start systems~\cite{serverlessllm,nvidia-streamer,hydraserve,lambdascale} optimize the reload cost but say nothing about the idle power of models that stay resident.
...
\begin{table}[t]
\caption{Phase~2 GPU configurations. All experiments used 30\,s sampling and 20-minute recording phases ($n=40$).}
\label{tab:gpus}
\centering
\small
\begin{tabular}{llrr}
\toprule
\textbf{GPU} & \textbf{Memory} & \textbf{TDP} & \textbf{VRAM range} \\
\midrule
...
Table~\ref{tab:cross} and Fig.~\ref{fig:hero} isolate the effect. On all three GPUs, creating a context triggers a discrete DVFS step of $+26.3$\,W (A100) to $+66.4$\,W (L40S); then \emph{sweeping VRAM from 0 to 40--72\,GB changes power by less than 1\,W}. The context accounts for $>99\%$ of the parking tax on every device. Per-device 95\% CIs for the VRAM slope are $[-0.005,+0.002]$ (H100), $[-0.002,-0.0005]$ (A100), and $[-0.019,+0.015]$ (L40S)\,W/GB. TOST rejects $|\beta| \ge 0.1$\,W/GB on every device ($p_{\text{TOST}} < 0.05$): we do not merely fail to find a memory effect, we formally establish its absence within bounds an order of magnitude below the DVFS step. The invariance holds across three memory subsystems with radically different bandwidths (3.35\,TB/s, 2.0\,TB/s, 864\,GB/s) and clocks (2619, 1512, 9001\,MHz)---exactly as the refresh-dominated physics of \S{II-B} predicts. Figure~\ref{fig:decomp} makes the decomposition visual: the model's contribution is invisible at scale.

\begin{table}[t]
\caption{Cross-architecture parking tax. $\beta$ is the VRAM regression slope across CUDA-active phases; all GPUs at 0\% utilization. The context term dominates; the model term is equivalent to zero.}
\label{tab:cross}
\centering
\small
\begin{tabular}{lrrr}
...
$\beta$ (W/GB) & $-$0.002 & $-$0.001$^\dagger$ & $-$0.002 \\
$p_{\text{TOST}}$ ($|\beta|<0.1$) & $10^{-9}$ & $<0.001$ & $<0.05$ \\
\midrule
\textbf{Context \% of tax} & \textbf{$>$99\%} & \textbf{$>$99\%} & \textbf{$>$99\%} \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$Significant but negative; thermal drift, not a memory effect (\S\ref{sec:a100}).}
\end{tabular}
\end{table}
...
A skeptic might expect real model weights---with their CUDA kernels, cuDNN handles, and tensors---to behave differently from \texttt{torch.empty}. They do not (Table~\ref{tab:validation}). On the H100, Qwen2.5-7B idles within 0.03\,W of a 16\,GB empty allocation. On the A100 and L40S, unloading 14.3\,GB of weights while keeping the context changes idle power by only $+0.21$ and $+0.47$\,W---within noise, far below the 26--66\,W context term. Absolute idle power varies across instances (105\,W on this A100 vs.\ 80\,W in Phase~2), tracking the 23\,W inter-node spread of Phase~1; the within-instance memory invariance is unaffected. Whether a GPU holds a real 7B model or an equivalent block of empty memory, the idle cost is the same.

\begin{table}[t]
\caption{Real-model validation. Qwen2.5-7B (fp16) idle power vs.\ a reference with different VRAM but the same CUDA context. All deltas within measurement noise.}
...
\midrule
\multirow{2}{*}{A100} & Qwen2.5-7B & 14.8 & 105.32 & --- \\
 & Post-unload (ctx only) & 0.5 & 105.53 & $+$0.21 \\
\midrule
\multirow{2}{*}{L40S} & Qwen2.5-7B & 14.8 & 97.07 & --- \\
 & Post-unload (ctx only) & 0.5 & 97.54 & $+$0.47 \\
\bottomrule
\end{tabular}
\end{table}
...
% ===========================================================================
Section~II identifies the CUDA context, acting through the DVFS boost, as the mechanism. We now establish this causally rather than by correlation.

\subsection{The control experiment}
This is a controlled test of causation. If idle power were determined by the model, then changing a driver setting while holding the model, its weights, and its VRAM footprint fixed would leave idle power unchanged. Instead, setting \perfboost to \texttt{1} holds the H100's SM clock at the bare-idle 345\,MHz even with an active context and 16\,GB resident, and idle power falls from 128.1\,W to 73.1\,W, indistinguishable from bare idle (73.3\,W). On the A100 SXM4 the flag recovers 96\% of the 11.1\,W overhead. Because the model is held fixed and only the driver flag changes, the 55\,W reduction is attributable to the CUDA context rather than to model size (Table~\ref{tab:flag}, Fig.~\ref{fig:flagpower}).
...
\caption{The control experiment, visualized. Hatched bars: flag enabled. On both H100 and A100 the flag collapses idle-with-context power to bare-idle levels---whether the context comes from \texttt{torch.empty} or a full vLLM stack with ${\sim}72$\,GB of model state. The model is identical across each pair; only the driver flag differs.}
\label{fig:flagpower}
\end{figure*}

\paragraph{No steady-state cost}
Through vLLM serving Qwen2.5-7B, steady-state latency is identical at every percentile through p99 on both architectures (Table~\ref{tab:flag})---vLLM's decode-phase CUDA graphs function correctly under the flag. The only cost is a one-time DVFS ramp on the first request after idle ($+147$\,ms H100, $+220$\,ms A100), gone by request~2 and eliminable with a keep-alive probe.
...
\label{eq:linear}
\end{equation}
so a 4-GPU node wastes ${\sim}216$\,W idle and an 8-GPU node ${\sim}430$\,W---determined by how many contexts are resident, not by the model or the interconnect. Every flag-on condition drops per-GPU idle to within 0.6\,W of bare idle. Warm latency is within run-to-run noise of baseline throughout.
...
\label{sec:default}
% ===========================================================================
The boosting behavior is an intentional default rather than an oversight. Determining the regime in which it is inappropriate requires weighing the latency it avoids against the energy it costs.
...
\label{eq:industry}
\end{equation}
With ${\sim}3.76$M datacenter GPUs shipped in 2023~\cite{nvidia-shipments}, indicative $\rho = 0.65$ (35\% idle~\cite{flexera}), and a fleet-weighted $\bar{P}_{\text{park}} = 40$\,W, the base estimate is 462\,GWh/year (${\sim}180$\,kT CO$_2$ at U.S. grid average). Varying each input over its plausible range (fleet 2.0--6.0M, $\rho$ 0.50--0.80, $\bar{P}_{\text{park}}$ 26.3--66.4\,W) bounds the range at 92--\num{1745}\,GWh/year. The dominant uncertainty is fleet composition and duty cycle---not the parking tax, which we measure directly. Even the 92\,GWh lower bound exceeds a small city's annual electricity use, and it is attributable to a single stale default.

% ===========================================================================
\section{Discussion and Limitations}
% ===========================================================================
\paragraph{Newer architectures and vendor power profiles}
We tested Hopper, Ampere, and Ada Lovelace but not Blackwell, to which we lacked access. The mechanism is the DVFS boost-on-context governor in the driver rather than an architecture-specific circuit, and it was invariant across the three generations we tested, differing only in magnitude (26--66\,W). Blackwell uses the same driver and DVFS stack, and the flag is architecture-agnostic, so we expect the anomaly to persist and the flag to apply; confirming this is future work. NVIDIA has introduced datacenter workload power profiles (including a Max-Q profile) on Blackwell B200~\cite{nvidia-power-profiles}, a coarse-grained, opt-in mechanism that selects hardware settings per workload class (training, inference, or HPC). By NVIDIA's description these profiles optimize \emph{active}-workload efficiency---redistributing power during computation---rather than idle power, and none targets the context-boost overhead measured here. The parking tax is therefore orthogonal to and unaddressed by current power profiles: a fine-grained, idle-side configuration artifact rather than a coarse-grained, active-side control. The \perfboost flag itself remains opt-in and off by default through the current driver series.
...
We test one device per architecture in Phase~2, complemented by 14~H100s in Phase~1, which bounds inter-device slope variation below measurement sensitivity. The flag was not tested on L40S (inventory constraints). Multi-GPU experiments cover a single 4$\times$H100 NVLink node; pipeline parallelism, 8-GPU nodes, NVSwitch fabrics, and multi-node deployments are untested, though the per-context mechanism suggests the per-GPU result is interconnect-independent. We measure one model size (Qwen2.5-7B); larger models may load differently, though memory-independence is tested to 72\,GB. The taxonomy rows other than the DVFS term are analytical, derived from documented mechanism, not measured here.

% ===========================================================================
\section{Conclusion}
% ===========================================================================
The cost of keeping a model warm in GPU memory is not a property of the model. Across HBM3, HBM2e, and GDDR6, in production and in controlled experiment, idle power is invariant to model size ($|\beta|<0.02$\,W/GB, equivalent to zero) and is instead a step function of a single control-plane state---whether a CUDA context is resident, which forces a 26--66\,W DVFS boost. A controlled experiment that holds the model fixed and toggles a single driver flag removes the overhead entirely, attributing it to the CUDA context rather than to model size. The behavior is an intentional latency-oriented default that is inappropriate for the always-on serving workloads that now dominate idle time, and it is the dominant member of a broader class of configuration-induced idle-power anomalies, for which we give a taxonomy and a detection principle. These results imply that size-based keep-warm and eviction heuristics optimize a variable that does not affect idle power; the relevant parameters are the number of resident contexts and the request arrival rate. At fleet scale the overhead represents an estimated 92--\num{1745}\,GWh/year, recoverable through a one-line configuration change.
